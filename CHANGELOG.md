...
- Encerramento documental do MOBS v1 no baseline `2ba5e8e2c3b6d2b85ca01c0db7f230da6ef19f78`: implementação concluída no alcance oficial Projeto → Marca associado (`2b0d8cfbf84256a846841c165b8272b3d376f71b`); nenhuma nova fatia executável exigida; Brand-only, Projeto sem Marca e demais variações permanecem abertos e não bloqueantes; Assets, Generators, Validators institucionais e Release fora da v1.
- Harmonização factual em `PROJECT_ENGINE.md`, `BRAND_ENGINE.md`, `PROJECT_CONTRACT.md` e `BRAND_CONTRACT.md`: removidas afirmações atuais de “apenas documentação”, “schema ainda não implementado” e “implementação não iniciada” onde ultrapassadas pela fatia v1; preservada a responsabilidade contratual de não definir schemas/serializações no próprio contrato.
